...
class Records extends CI_Controller {
	...
	<script type=\"text/javascript\">
		...
		// comptage des enregistrments par catégories
	...
	public function find($start = 0) {
		...
		// récupération de la chaîne de texte saisie dans le header
		$header_string = $this->input->post('search_string');
		// la chaîne saisie dans le header doit contenir au moins 2 caractères
		if (strlen($header_string) < 2 and $this->uri->segment(3) == "") $header_string = "xxxxx";
		// initialisation, passage et récupération de la chaîne dans l'url
		if ($this->uri->segment(3) == "") {
			// échappement de la chaîne saisie dans le header
			$string = $this->db->escape_like_str($header_string);
			$url_string = urlencode($header_string);
		} else {
			$url_string = $this->uri->segment(3);
			// échappement de la chaîne récupérée dans l'url
		...
		*/
		...
		// acquisition d'un paquet de [$qty_per_page] enregistrements en commençant à [$start]
	...
	public function find_old($start = 0) {
		...
		$data['titre'] = $this->all_model->add_nav_to_title($data['bandeau']);
		$data['current_user_rights'] = $this->session->userdata('user_rights');
		// récupération de la chaîne de texte (saisie dans le header OU passée en url)
		$header_string = $this->input->post('search_string');
		
		// contrôle de la chaîne avec restriction
		// 																					à compléter !!!
		$unwanted = array(' ', '\\', '\'');
		$header_string = str_replace($unwanted, '', $header_string);
		
		if ((strlen($header_string) > 1)) {
			// la chaîne $header_string doit être échappée
			$string = $this->db->escape_like_str($header_string);
		} else {
			// récupération de la chaîne dans l'url
	...
	public function detail($uid) {
		...
		// incrémentation du champ clicks
	...
	<script type=\"text/javascript\">
		...
		// acquisition des infos liées : groupe, pays
		...
		// recherche de la photo liée à l'enregistrement ou à défaut du logo de la catégorie - Un lien permet de télécharger une photo ou d'en changer
		$data['record']['associated_image'] = $this->records_model->get_associated_image($itemid, $uid, 'detail');
		// construction des blocs qui seront affichés
		switch ($itemid) {
			case "1":
				// bloc action :: "Imprimer une étiquette d'expédition"
		...
		// affichage des infos collectées dans la vue appropriée
	...
	public function deep_find() {
		...
		// acquisition des lignes qui répondent aux critéres de recherche passés en _POST
	...
	public function edit($uid) {
		// acquisition de la ligne concernée
		$data['record'] = $this->all_model->get_fullrow('input', 'uid', $uid);
		if (empty($data['record'])) {
			show_404();
		}
	...
	public function delete($uid) {
		...
		if ($this->session->userdata('user_rights') == "ReadOnly") {
			...
			// effacement du fichier photo associé
	...
	public function save() {
					//							print_r($_POST);
		// récupération de la catégorie et de l'identifiant
		$itemid = $this->input->post('itemid');
		...
		// aiguillage en fonction de la catégorie
		switch ($itemid) {
			case "1":
				// définition des règles de validation
				$this->form_validation->set_rules('title', '« title »', 'max_length[64]');								// non mandatory
				$this->form_validation->set_rules('first_name', '« '.lang('label_first_name').' »', 'required');
				$this->form_validation->set_rules('last_name', '« '.lang('label_last_name').' »', 'required');
				$this->form_validation->set_rules('birthday', '« '.lang('label_birthday').' »', 'max_length[64]');		// date
				$this->form_validation->set_rules('groupid', '« groupid »', 'max_length[64]');						// select
				$this->form_validation->set_rules('adress1', '« adress1 »', 'max_length[64]');						// non mandatory
				$this->form_validation->set_rules('adress2', '« adress2 »', 'max_length[64]');						// non mandatory
				$this->form_validation->set_rules('postcode', '« postcode »', 'max_length[64]');						// non mandatory
				$this->form_validation->set_rules('city', '« city »', 'max_length[64]');								// non mandatory
				$this->form_validation->set_rules('country_id', '« country_id »', 'max_length[64]');				// select
				$this->form_validation->set_rules('phone_home', '« phone_home »', 'max_length[64]');				// non mandatory
				$this->form_validation->set_rules('phone_work', '« phone_work »', 'max_length[64]');				// non mandatory
				$this->form_validation->set_rules('phone_cell', '« phone_cell »', 'max_length[64]');				// non mandatory
				$this->form_validation->set_rules('fax', '« fax »', 'max_length[64]');									// non mandatory
				$this->form_validation->set_rules('email', '« email »', 'valid_email');									// non mandatory
				$this->form_validation->set_rules('gmap', '« gmap »', 'max_length[2048]');								// non mandatory
				$this->form_validation->set_rules('memo', 'memo', 'min_length[8]');										// non mandatory
				$this->form_validation->set_rules('visibility', '« visibility »', 'required');						// radio
				...
				if ($this->form_validation->run() == FALSE){
					...
					// succès : récupération (et traitement) des données passées en post
					...
					// test de l'action à effectuer : modification ou création
					if($uid == "not_set") {
			...
			case "2":
				// définition des règles de validation
				$this->form_validation->set_rules('first_name', '« '.lang('label_note_title').' »', 'required');
				$this->form_validation->set_rules('groupid', 'groupid', 'max_length[64]');
				$this->form_validation->set_rules('memo', '« Note »', 'required|min_length[8]');
				$this->form_validation->set_rules('visibility', '« visibility »', 'required');				// radio
				// test de validation du formulaire
				if ($this->form_validation->run() == FALSE){
					// échec : retour au formulaire
					...
					// succès : récupération des données passées en post
					$data['first_name']		= $this->input->post('first_name');
					$data['itemid']			= $this->input->post('itemid');
					$data['groupid']			= $this->input->post('groupid');
					$data['memo']				= $this->input->post('memo');
			...
			case "3":
				// définition des règles de validation
				$this->form_validation->set_rules('first_name', '« '.lang('label_doc_name').' »', 'required');
				$this->form_validation->set_rules('last_name', '« last_name »', 'max_length[64]');
				$this->form_validation->set_rules('birthday', '« birthday »', 'max_length[64]');
				$this->form_validation->set_rules('groupid', 'groupid', 'max_length[64]');
				$this->form_validation->set_rules('url', '« '.lang('label_doc_url').' »', 'required');
				$this->form_validation->set_rules('width', '« width »', 'max_length[64]');
				$this->form_validation->set_rules('height', '« height »', 'max_length[64]');
			...
			case "4":
				// définition des règles de validation
				$this->form_validation->set_rules('first_name', '« '.lang('label_site_name').' »', 'required');
				$this->form_validation->set_rules('groupid', 'groupid', 'max_length[64]');
				$this->form_validation->set_rules('username', '« username »', 'max_length[64]');
				$this->form_validation->set_rules('email', '« email »', 'max_length[64]');
				$this->form_validation->set_rules('password', '« password »', 'max_length[64]');
				$this->form_validation->set_rules('url', '« '.lang('label_site_url').' »', 'required');
				$this->form_validation->set_rules('width', '« width »', 'max_length[64]');
				$this->form_validation->set_rules('height', '« height »', 'max_length[64]');
				$this->form_validation->set_rules('memo', 'memo', 'min_length[8]');
				$this->form_validation->set_rules('visibility', '« visibility »', 'required');				// radio
	...
	public function group_liste($itemid) {
		...
		if ($this->session->userdata('user_rights') != "SuperAdmin") {
			...
			// acquisition des groupes de la catégorie concernée
	...
	public function group_add() {
		...
		$itemid = $this->uri->segment(3);
		$category = $this->records_model->name_category($itemid);
		// collecte des infos à envoyer à la vue
	...
	public function group_save($gid) {
		// sélection du mode (création ou modification)
		if ($gid != 'not_set') {
			// modification : recherche du groupe concerné
			$group = $this->records_model->get_group_by_id($gid);
			$itemid = $this->records_model->get_category_of_group($gid);
			$category = $this->records_model->name_category($itemid);
			// collecte des infos à envoyer à la vue
			...
			// création : récupération des infos passées en _POST
			...
			// message de confirmation dans la fenêtre popup
	...
	public function group_delete_request($gid) {
		...
		// recherche des enregistrements éventuels liés à ce groupe
	...
	public function group_delete($gid) {
		...
		$data['bandeau'] = lang('action_delete_group') . " : " . $group ;
		$data['titre'] = $this->all_model->add_nav_to_title("<span class=\"gris_moyen\">" . $category . " : </span>" . lang('action_delete_group') . " : <br />" . $group);
		// Suppression du groupe et redirection vers la page qui liste les groupes de cette catégorie
	...
	public function most($start = 0) {
		...
		// prévention contre une fausse url
		if (($most != 'recent') and ($most != 'visited')) {								// à compléter !!!
			show_404();
		}
		// mise en place de la pagination
	...
	public function print_etic_exp($uid) {
		// acquisition des infos expéditeur
		...
		// incrémentation du champ clicks du destinataire
	...
	public function photo($mode, $uid) {
		...
		if ($this->session->userdata('user_rights') == "ReadOnly") {
			...
			if ($mode == 'upload') {
				...
				// avertissement à propos de l'effacement de la photo précédente
	...
	public function upload_photo($uid) {
		...
		if ($mode == 'upload') {
			...
			$data['bandeau']  = lang('action_change_photo') ;
			$data['titre'] = "<div class=\"entete\">" . lang('action_change_photo') . "</div>\r\n";
			// effacement de la photo précédente avec message informatif
			if ($this->records_model->photo_file_delete($itemid, $uid)) {
