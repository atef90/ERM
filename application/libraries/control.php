...
class Control extends CI_Controller {
	// déclaration de la variable CI
	var $CI;
	// constructeur
	public function __construct() {
		// instanciation de la variable CI
		$this->CI =& get_instance();
		// chargement des librairies, modèles, fichiers_lang
	...
	public function ask_access() {
		if ($this->CI->session->userdata('user_id')) {
			...
			// vérification de la validité des sa session
			if ($this->CI->control->is_valid_session()) {
				...
				// désactivation de la session et message de warning
